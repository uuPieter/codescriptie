...
int main (int argc, char ** argv) {
  ...
  Selector parton_selector = SelectorVertexNumber(0);
  ...
  cout << "Value of nEvent: " << nEvent << endl; // Debugging statement
  // loop over events
  int iev = 0;
  unsigned int entryDiv = (nEvent > 200) ? nEvent / 200 : 1;
  while (iev < nEvent || nEvent == -1)  {
     bool has_next_event = mixer.next_event();
    if (!has_next_event) {
        cout << "No more events to process. Exiting loop." << endl;
        continue;
    }
    if (iev == nEvent) {
      cout << "event is over" << endl;
      break;
    }
    ...
    for (const auto& parti : particlesMergedAll) {
      if (Ptmin_selector(parti) && charged_selector(parti) && parton_selector(parti)) {
